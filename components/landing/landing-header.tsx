        ...
        <div className="hidden md:flex items-center gap-1">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className={`relative px-3 py-1.5 text-sm rounded-full transition-all duration-300 group ${
                isActive(item.href)
                  ? 'text-foreground'
            ...
            >
              ...
              {/* Bottom line */}
              <span
                className={`absolute bottom-1 left-1/2 -translate-x-1/2 h-px bg-linear-to-r from-orange-500 to-amber-500 transition-all duration-300 ${
                  isActive(item.href) ? 'w-6' : 'w-0 group-hover:w-6'
                }`}
              />
              {/* Active bg glow */}
              {isActive(item.href) && (
                <span className="absolute inset-0 rounded-full bg-orange-500/10 border border-orange-500/25 z-0" />
              )}
            </a>
          ))}
        </div>
            ...
            {navItems.map((item) => (
              <a
                ...
                className={`px-4 py-2.5 rounded-xl text-sm transition-all duration-200 ${
                  isActive(item.href)
                    ? 'bg-orange-500/15 border border-orange-500/25 text-orange-600 dark:text-orange-400 font-medium'
