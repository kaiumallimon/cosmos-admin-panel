...
async function getHandler(
  ...
  try {
    ...
    // Fetch thread history from the external API
    const response = await fetch(`${SERVER_BASE_URL}/chat/threads/${thread_id}/history`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { detail: errorData.detail || errorData.error || 'Failed to fetch thread history' },
...
export const GET = withAuthAny(getHandler);
